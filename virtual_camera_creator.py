...
def extractor(src_file: str, dst_dim: Optional[tuple] = None,
    ...
    if dst_postfix is not None or dst_postfix != "":
        folder_name += "_%s" % (dst_postfix,)
    ...
    for i in tqdm(range(start, end + 1)):
        ...
        # 保存bmp
        # 补零
        dst_path = os.path.join(dst_dir, "%s.bmp" % str(i).zfill(figures))
...
if __name__ == "__main__":
    ...
    extractor(src_file=src, dst_dim=dim, start_frame=None, end_frame=None, step_frame=s, is_gray=True, flip=f, show_ratio=0.3, dst_postfix=postfix)
